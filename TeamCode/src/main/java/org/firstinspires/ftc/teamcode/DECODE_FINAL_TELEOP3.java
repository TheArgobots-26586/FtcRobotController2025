...
@TeleOp(name="DECODE_FINAL_TELEOP3", group="Robot")
public class DECODE_FINAL_TELEOP3 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (result != null && result.isValid() && result.getTx() != tx) {
                ...
                tx = result.getTx();
                if (detectedID == 20) {
                    double val = Math.min(Math.max(0.68, turrpos + (tx / 360)-0.005), 0.88);
