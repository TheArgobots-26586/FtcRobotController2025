...
@TeleOp(name="DECODE_FINAL_TELEOP4_SNAP", group="Robot")
public class DECODE_FINAL_TELEOP4_SNAP extends LinearOpMode {
...
//ELECTRONIC SETUP
    ...
    Range2 range4 = new Range2(112, Integer.MAX_VALUE, 1430);
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (result != null && result.isValid() && runtime.milliseconds() - lastTime > 50) {
                lastTime = runtime.milliseconds();
                int detectedID = result.getFiducialResults().get(0).getFiducialId();
                tx = result.getTx();
                double val = Math.min(Math.max(0.68, turrpos + (tx / 360)), 0.88);
                boolean inrange = Math.abs(turrpos-val)>0.005;

                if (detectedID == 20 && inrange) {
                    //  double val = Math.min(Math.max(0.75, turret.getPosition() + (tx / 360)), 1);
                    if (val > turrpos) turret.setPosition(Math.max(0.68, Math.min(turrpos+0.001, 0.88)));
                    if (val < turrpos) turret.setPosition(Math.max(0.68, Math.min(turrpos-0.001, 0.88)));
                    //sleep(900);
                    telemetry.addData("servo target pos blue", val );
                    telemetry.update();
                } if (detectedID == 24 && inrange) {
                    //  double val = Math.min(Math.max(0.75, turret.getPosition() + (tx / 360)), 1);
